...
void Droga::setTexture(std::vector<sf::Texture*>& _texture_dekoracje, sf::Texture*& _texture_droga)
{
	for (int i = 0; i < this->rozmiar.y; ++i)
		for (int j = 0; j < this->rozmiar.x; ++j)
			if (this->sprite_dekoracje[i][j].getTexture() != nullptr) {
				int wybrana_dekoracja = rand() % 100 + 1;
				if (wybrana_dekoracja <= 70)
					wybrana_dekoracja = 0;
				else if (wybrana_dekoracja <= 80)
				...
				this->sprite_dekoracje[i][j].setTexture(*_texture_dekoracje[wybrana_dekoracja]);
			}
